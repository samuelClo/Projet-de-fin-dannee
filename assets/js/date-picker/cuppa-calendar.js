(function(window){
  'use strict';
	...
	let cal_days_labels = ['Su', 'Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa'];
	let cal_full_days_lables = ["Sunday","Monday","Tuesday","Wednesday","Thursday","Friday","Saturday"];

	// these are human-readable month name labels, in order
	let cal_months_labels = ['January', 'February', 'March', 'April',
	                     'May', 'June', 'July', 'August', 'September',
	                     'October', 'November', 'December'];
	let cal_months_labels_short = ['JAN', 'FEB', 'MAR', 'APR',
		                     'MAY', 'JUN', 'JUL', 'AUG', 'SEP',
		                     'OCT', 'NOV', 'DEC'];
	...
	let extend = function(to, from, overwrite)
    {
        let prop, hasProp;
        for (prop in from) {
	...
	WinkelCalendar.prototype.createMonthView = function(){
			...
			for(let k=0;k<cal_months_labels.length;k++){
	...
	WinkelCalendar.prototype.onMonthSelect = function(e){
			e.stopPropagation();
			let month = cal_months_labels_short.indexOf(e.target.getAttribute('value'));
	...
	WinkelCalendar.prototype.setPrevYearList = function(){
		let startYear = parseInt(this.el.calendarPopup.yearsView.yearsList[0].textContent - 9);
		let currentYear = this.date.getFullYear();
		for(let k=0; k< 9; k++){
	...
	WinkelCalendar.prototype.setNextYearList = function(){
		let startYear = parseInt(this.el.calendarPopup.yearsView.yearsList[8].textContent) + 1;
		let currentYear = this.date.getFullYear();
		for(let k=0; k< 9; k++){
			if((startYear + k) === currentYear ){
					this.el.calendarPopup.yearsView.yearsList[k].setAttribute('class','current-year');
	...
	WinkelCalendar.prototype.updateMonthView = function(){
		let months = this.el.calendarPopup.monthsView.children;
	...
	WinkelCalendar.prototype.createHeaderView = function(){
		  ...
		  let self = this;
		  let year = self.date.getFullYear(),
		...
		// get first day of month
		  ...
		  let monthLength = cal_days_in_month[self.date.getMonth()];
		  ...
		  // do the header
		  let monthName = cal_months_labels[month];
	...
	WinkelCalendar.prototype.createDaysView = function(){
		...
		// get first day of month
		  ...
		  // find number of days in month
		  let monthLength = this.getMonthLength(month,year);
		  
		  // do the header

		  let daysTable = document.createElement("table");
	...
	WinkelCalendar.prototype.attachEvents = function(){
		let self = this;
		let elem = self.el.calendarPopup.daysTable;
	...
	WinkelCalendar.prototype.prevMonth = function(e){
		...
		if(self.date.getMonth() == 0){
			...
			let prevmonthLength = self.getMonthLength(self.date.getMonth() - 1, self.date.getFullYear());
			let currentDate = self.date.getDate();
	...
	WinkelCalendar.prototype.nextMonth = function(e){
		...
		if(self.date.getMonth() == 11){
			...
			let nextmonthLength = self.getMonthLength(self.date.getMonth() + 1, self.date.getFullYear());
			let currentDate = self.date.getDate();
